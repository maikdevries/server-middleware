...
/**
 * A middleware chain that can be extended with {@link Middleware} and {@link Chain} or ended with {@link Handler}.
 *
...
type Chain<R, P> = {
	 ...
	 */
	end<RS>(handler: Handler<RS>): Handler<Reduce<R, RS, P>>;
};

/**
 * Composes two {@link Middleware} or a {@link Middleware} and a {@link Chain} into a single {@link Middleware}.
 *
 ...
 * @template RS - Context properties that the second middleware or chain requires
 * @template PS - Context properties that the second middleware or chain provides
 *
 * @param first - First {@link Middleware}
 * @param second - Second {@link Middleware} or {@link Chain}
 * @returns Composed {@link Middleware} with updated context object types
 */
function composeMiddleware<RF, PF, RS, PS>(
	first: Middleware<RF, PF>,
	second: Middleware<RS, PS> | Chain<RS, PS>,
...
/**
 * Composes a {@link Middleware} and a {@link Handler} into a single {@link Handler}.
 *
 * The middleware receives the original context, the handler receives the context modified by the middleware.
 *
 * @template RF - Context properties that the middleware requires
 * @template PF - Context properties that the middleware provides
 * @template RS - Context properties that the handler requires
 *
 * @param middleware - The {@link Middleware}
 * @param handler - The {@link Handler}
 * @returns Composed {@link Handler} with updated context object types
 */
function composeHandler<RF, PF, RS>(middleware: Middleware<RF, PF>, handler: Handler<RS>): Handler<Reduce<RF, RS, PF>> {
	return (request, context) => middleware(request, context, (r, c) => handler(r, c as RS));
}

/**
 * Creates a middleware chain that allows composing (multiple) {@link Middleware} and a {@link Handler}.
 *
 * @template R - Context properties that the middleware requires
 * @template P - Context properties that the middleware provides
 *
 * @param middleware - {@link Middleware} to make chainable
 ...
 *
 * const app = chain(logger).add(authorise).end(handler);
 * ```
 */
export function chain<R, P>(middleware: Middleware<R, P>): Chain<R, P> {
	const copy = middleware.bind(null) as Chain<R, P>;

	copy.add = <RS, PS>(next: Middleware<RS, PS> | Chain<RS, PS>) => chain(composeMiddleware(middleware, next));
	copy.end = <RS>(handler: Handler<RS>) => composeHandler(middleware, handler);
